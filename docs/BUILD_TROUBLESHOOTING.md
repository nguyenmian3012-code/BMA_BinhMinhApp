...
| Deployment review | Tài liệu định tuyến staging tới `8791` nhưng Compose cố định host port `8790` | Cổng host chưa cấu hình theo môi trường; staging có nguy cơ đụng production | Thêm `BMA_HOST_PORT`, dùng `bma-staging`/`bma-production` làm Compose project riêng và kiểm tra cả hai cấu hình trong CI | Staging phải render port `8791` + `/bmapp-staging`; production render `8790` + `/bmapp` |
| Browser session review | Admin dùng cookie `__Host-` trong khi BMA chạy dưới PathBase | `__Host-` bắt buộc cookie path `/`; cookie scoped `/bmapp` có thể bị trình duyệt từ chối | Production dùng `__Secure-` + cookie path đúng PathBase; CI đăng nhập Razor Admin bằng antiforgery thật | Smoke test phải nhận cookie path `/bmapp-ci` và truy cập được trang Admin có bảo vệ |
