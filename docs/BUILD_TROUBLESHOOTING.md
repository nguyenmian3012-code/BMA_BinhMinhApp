...
| Native runtime review | Database đã chuyển sang PostgreSQL Windows nhưng script staging vẫn tạo PostgreSQL container | Hai nhánh phát triển tách sau commit migration; runtime cũ bị giữ lại | Bỏ Compose trên MinhComp; chạy BMA bằng Windows Service với database native riêng | Health phải trả `runtime=windows-service`, `database=postgresql-native` |
