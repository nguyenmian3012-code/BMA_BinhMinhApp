...
function Invoke-NativePsql([string]$Sql, [switch]$TuplesOnly) {
    $args = @(
        '-X', '-v', 'ON_ERROR_STOP=1',
        '-h', '127.0.0.1', '-p', "$NativePort",
        '-U', $NativeAdminUser, '-d', $NativeDatabase
    )
...
function Get-DockerSubnets {
    ...
    # Docker Desktop commonly reaches Windows through the 192.168.64.0/20 backend range.
...
function Write-SecureComposeEnv([hashtable]$AppEnv, [string]$RuntimePassword) {
    ...
    try {
        ...
        foreach ($id in $ids) {
            $acl.AddAccessRule((New-Object Security.AccessControl.FileSystemAccessRule($id, $rights, $allow)))
        }
        Set-Acl -Path $path -AclObject $acl
    }
    catch {
        throw "Could not secure production credential file '$path': $($_.Exception.Message)"
    }
...
$postgresService = Get-Service -ErrorAction SilentlyContinue |
    Where-Object { $_.Name -eq 'postgresql-x64-17' -or $_.Name -like 'postgresql*17*' } |
...
$bmaIds = @(docker ps -q --filter "publish=$BmaHostPort" | Where-Object { $_ })
if ($bmaIds.Count -eq 0) {
    $bmaIds = @(docker ps -q --filter 'label=com.docker.compose.service=bma' | Where-Object { $_ })
}
if ($bmaIds.Count -ne 1) {
    throw "Expected exactly one running production BMA container; found $($bmaIds.Count)."
}
...
if ($pgIds.Count -ne 1) {
    throw "Expected one running PostgreSQL container in project '$project'; found $($pgIds.Count)."
}
...
if ($sourceDb -notmatch '^[A-Za-z0-9_]+$' -or $sourceUser -notmatch '^[A-Za-z0-9_]+$') {
    throw 'Unsafe source DB/user name.'
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or $dbCheck -ne '1') {
        throw "Native database '$NativeDatabase' does not exist or cannot be reached."
    }
    ...
    if ($already -eq '1') {
        throw 'Target public schema already contains BMA EF migration history; refusing a duplicate direct import.'
    }
    ...
    if ($roleExists -ne '1') {
        [void](Invoke-NativePsql 'CREATE ROLE bma_runtime LOGIN;')
    }
    ...
    foreach ($subnet in $subnets) {
        ...
        if ($existingHba -notmatch [regex]::Escape($line)) {
    ...
    New-NetFirewallRule -DisplayName $fwName -Direction Inbound -Action Allow -Protocol TCP `
        -LocalPort $NativePort -RemoteAddress $subnets -Profile Any | Out-Null

    Restart-Service $postgresService.Name
    Start-Sleep -Seconds 3

    $env:PGPASSWORD = $runtimePassword
    $networkTest = (docker run --rm -e PGPASSWORD postgres:17-alpine psql `
        -h host.docker.internal -p "$NativePort" -U bma_runtime -d $NativeDatabase -tA -c 'SELECT 1;' | Out-String).Trim()
    if ($LASTEXITCODE -ne 0 -or $networkTest -ne '1') {
        throw 'Docker -> native PostgreSQL connectivity test failed. Current BMA has not been stopped.'
    }
    Write-Host 'Docker -> native PostgreSQL: PASS' -ForegroundColor Green

    $env:PGPASSWORD = $adminPassword
    Write-Host "Stopping current BMA container to freeze writes: $($oldInfo.Name.TrimStart('/'))" -ForegroundColor Yellow
    ...
    try {
        ...
        if (-not $migrationCount -or [int]$migrationCount -lt 1) {
            throw 'BMA EF migration history was not restored.'
        }
        ...
        try {
            ...
            $image = $oldInfo.Image
            $dpMount = @($oldInfo.Mounts | Where-Object { $_.Destination -eq '/home/app/.aspnet/DataProtection-Keys' }) |
                Select-Object -First 1

            $runArgs = @(
                'run', '-d', '--name', $NewContainerName,
                '--restart', 'unless-stopped',
                '-p', "127.0.0.1:${BmaHostPort}:8080",
                '--read-only', '--tmpfs', '/tmp',
            ...
            if ($LASTEXITCODE -ne 0 -or -not $newId) {
                throw 'Failed to start native-DB BMA container.'
            }
        }
        finally {
        ...
        if (-not $healthy) {
            throw "New production BMA health check failed: $healthUrl"
        }
        ...
        Write-Host "Production endpoint remains 127.0.0.1:$BmaHostPort; gateway routing does not change."
        Write-Host 'Legacy Docker PostgreSQL volume was not deleted, but it is inactive and cannot restart automatically.'
    }
    catch {
